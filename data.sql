...
-- ==================== 订购数据（v3.0 单表） ====================
DECLARE @op INT = (SELECT UserId FROM Users WHERE Username = 'stockop');
DECLARE @d1 INT = (SELECT DemandId FROM BookDemands WHERE DemandTitle = N'2024秋季学期教材需求');
DECLARE @d2 INT = (SELECT DemandId FROM BookDemands WHERE DemandTitle = N'计算机系补充教材');
